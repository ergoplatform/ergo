...
class ErgoMiner(ergoSettings: ErgoSettings,
  ...
  private def receiveSemanticallySuccessfulModifier: Receive = {
    case SemanticallySuccessfulModifier(mod) =>
      //Case when we are already mining by the time modifier arrives
      if (isMining) {
        mod match {
          /**
            * Got block from node view that has header's id which isn't equals to our candidate's parent id.
          ...
          case f: ErgoFullBlock if !candidateOpt.flatMap(_.parentOpt).exists(_.id sameElements f.header.id) =>
            produceCandidate
          //We do not need to react for non-fullblock modifiers
          case _ =>
        }
      //Non obvious but case when mining is enabled, but miner doesn't start yet. Initialization case.
        ...
        mod match {
          ...
          case f: ErgoFullBlock if f.header.timestamp >= startTime =>
            self ! StartMining
          //We do not need to react for non-fullblock modifiers even if we don't start to mine
          case _ =>
        }
      }
  }
