...
import org.ergoplatform.{ErgoBox, ErgoConstants}
...
object ValidationRules {
  ...
  lazy val rulesSpec: Map[Short, RuleStatus] = Map(
    ...
    txScriptValidation -> RuleStatus(s => fatal(s"Scripts of all transaction inputs should pass verification. $s"),
      Seq(classOf[ErgoTransaction]),
      mayBeDisabled = false),
    txAdditionalRegistersCount -> RuleStatus(s => fatal(s"Additional registers count should not exceed " +
      s"${ErgoBox.nonMandatoryRegistersCount}. $s"),
      Seq(classOf[ErgoTransaction]),
      mayBeDisabled = false),
    txMaxBoxSize -> RuleStatus(s => fatal(s"Box size should not exceed ${ErgoConstants.MaxBoxSize.get}. $s"),
      Seq(classOf[ErgoTransaction]),
      mayBeDisabled = false),
    txRegisterData -> RuleStatus(s => fatal(s"Additional registers data should pass verification. $s"),
      Seq(classOf[ErgoTransaction]),
      mayBeDisabled = false),
  ...
  val txAdditionalRegistersCount: Short = 120
