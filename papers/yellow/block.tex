...
It contains 2 parts:
\begin{itemize}
    \item{\em Mandatory fields } - fields which keys and values are set via consensus rules (so they may be changed
    via soft/hard forks only). These fields have one-byte key and at most 32 bytes value.

    \knote{How to add new keys via soft-fork?}

    \item{\em Optional fields } - random data miner may add to a block. This section contains at most 2
    elements with 32-bytes key size and at most 64-bytes value size.
\end{itemize}
