  ...
  def validateStateful(boxesToSpend: IndexedSeq[ErgoBox],
    ...
    ModifierValidator(stateContext.validationSettings)
      ...
      validationState
        ...
        .validate(txFuture, out.creationHeight <= stateContext.currentHeight, s"id: output ${Algos.encode(out.id)}, ${out.creationHeight} <= ${stateContext.currentHeight}")
        ...
        ErgoTransaction.extractAssets(boxesToSpend) match {
          case Success((inAssets, inAssetsNum)) =>
            ...
            validation
              ...
              .validate(bsBlockTransactionsCost, newRemainingCost >= 0, s"$id: assets cost")
      ...
      val currentTxCost = validation.result.payload.get
      val newRemainingCost = remainingCost - currentTxCost
      ...
      val ctx = new ErgoContext(stateContext, transactionContext, proverExtension, newRemainingCost, initialCost)
