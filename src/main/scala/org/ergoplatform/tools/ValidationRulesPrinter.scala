...
object ValidationRulesPrinter extends App with ScorexLogging {
  ...
  rules.toSeq.sortBy(_._1).foreach { r =>
    ...
    val rule = r._2.invalidMod(InvalidModifier("", emptyModifierId, ModifierTypeId.fromByte(0))).errors.head.message.trim
    ...
    if (r._2.invalidMod(InvalidModifier("", emptyModifierId, ModifierTypeId.fromByte(0))).isFatal) {
