...
class CandidateGenerator(
  ...
  private def initialized(state: CandidateGeneratorState): Receive = {
    ...
    case gen @ GenerateCandidate(txsToInclude, reply, _, optPk) =>
      ...
      if (cachedFor(state.cachedCandidate, txsToInclude, effectiveMinerPk)) {
...
object CandidateGenerator extends ScorexLogging {
  ...
  def cachedFor(
    ...
    candidateOpt.isDefined && candidateOpt.exists { c =>
      val sameMinerPk = c.externalVersion.pk == minerPk
      val sameTxs = txs.isEmpty || (txs.size == c.txsToInclude.size && txs.forall(c.txsToInclude.contains))
      sameMinerPk && sameTxs
    }
  }
