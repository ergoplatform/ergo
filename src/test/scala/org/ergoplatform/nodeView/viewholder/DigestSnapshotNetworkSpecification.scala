  ...
  Seq((StateType.Utxo, "requested"), (StateType.Digest, "requested"),
    ...
    property(s"${mode.stateTypeName} snapshot chunk receive handles $input peer messages") {
      ...
      try {
        ...
        history.registerManifestToDownload(manifest, fixture.manifest, 19, Seq.empty)
        ...
        chunkIds.indices.foreach { index =>
          history.contains(bytesToId(UtxoSetSnapshotProcessor.snapshotScanChunkKey(index))) shouldBe false
        }
