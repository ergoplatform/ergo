...
object CandidateGenerator extends ScorexLogging {
  ...
  def createCandidate(
    ...
    Try {
      ...
      val nodeVersionExtension = ExtensionCandidate(
        Seq(Array(Extension.MinerVersionPrefix, 0: Byte) -> Version.VersionString.getBytes("UTF-8"))
      )
      val interlinksPlusNodeVersionExtension = interlinksExtension ++ nodeVersionExtension
      val votingSettings      = ergoSettings.chainSettings.voting
      val (extensionCandidate, votes: Array[Byte], version: Byte) = bestHeaderOpt
        .map { header =>
          ...
          if (newHeight % votingSettings.votingLength == 0 && newHeight > 0) {
            ...
            (
              newParams.toExtensionCandidate ++ interlinksPlusNodeVersionExtension ++ newValidationSettings.toExtensionCandidate,
            ...
            (
              interlinksPlusNodeVersionExtension,
              votes,
        ...
        .getOrElse(
          (interlinksPlusNodeVersionExtension, Array(0: Byte, 0: Byte, 0: Byte), Header.InitialVersion)
        )
