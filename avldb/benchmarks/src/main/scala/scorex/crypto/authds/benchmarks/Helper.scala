...
object Helper {
  ...
  def generateOps(r: Range): Array[Operation] = {
    ...
    val inserts = (r.head until r.head + insertsCount).map { i =>
      ...
      val k = Longs.toByteArray(i.toLong) ++ Longs.toByteArray(System.currentTimeMillis)
      k.copyToArray(key)
  ...
  def persistentProverWithVersionedStore(initialKeepVersions: Int,
    ...
    val storage = new VersionedLDBAVLStorage[Digest32, HF](store, AvlTreeParameters(kl, Some(vl), ll))
