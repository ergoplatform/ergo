...
trait Stubs extends ErgoTestHelpers with TestFileUtils {
  ...
  def generateHistory(verifyTransactions: Boolean = true,
    ...
    val nodeSettings: NodeConfigurationSettings = NodeConfigurationSettings(stateType, verifyTransactions, blocksToKeep,
      UtxoSettings(false, 0, 2), NipopowSettings(poPoWBootstrap, 1), mining = false, txCostLimit, txSizeLimit, blockCandidateGenerationInterval = 30.seconds, useExternalMiner = false,
