...
object ModifierValidator {
  ...
  def error(error: String, modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value): Invalid =
    invalid(new RecoverableModifierError(error, modifierId, modifierTypeId, None))

  /** report recoverable modifier error that could be fixed by later retries */
  def error(error: String, modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value, cause: Throwable): Invalid =
    invalid(new RecoverableModifierError(msg(error, cause), modifierId, modifierTypeId, Option(cause)))

  /** report recoverable modifier error that could be fixed by later retries */
  def error(description: String, detail: String, modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value): Invalid =
  ...
  def fatal(error: String, modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value): Invalid =
    invalid(new MalformedModifierError(error, modifierId, modifierTypeId, None))

  /** report non-recoverable modifier error that could not be fixed by retries and requires modifier change */
  def fatal(error: String, modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value, cause: Throwable): Invalid =
    invalid(new MalformedModifierError(msg(error, cause), modifierId, modifierTypeId, Option(cause)))

  /** report non-recoverable modifier error that could not be fixed by retries and requires modifier change */
  def fatal(description: String, detail: String, modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value): Invalid =
...
case class ValidationState[T](result: ValidationResult[T], settings: ValidationSettings)(implicit e: ScorexEncoder) {
  ...
  /** Validate the first argument equals the second. This should not be used with `ModifierId` of type `Array[Byte]`.
    ...
    */
  def validateEquals[A](id: Short, given: => A, expected: => A, modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value): ValidationState[T] = {
  ...
  /** Validate the `id`s are equal. The `error` callback will be provided with detail on argument values
    */
  def validateEqualIds(id: Short, given: => ModifierId, expected: => ModifierId, modifierTypeId: ModifierTypeId.Value): ValidationState[T] = {
    pass {
  ...
  /** Validate the `condition` is `Success`. Otherwise the `error` callback will be provided with detail
    ...
    */
  def validateNoFailure(id: Short, condition: => Try[_], modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value): ValidationState[T] = {
  ...
  /**
    ...
    */
  def validateNoFailure(id: Short, condition: => Try[_], modifierTypeId: ModifierTypeId.Value): ValidationState[T] = {
    pass {
  ...
  /** Validate the `block` doesn't throw an Exception. Otherwise the `error` callback will be provided with detail
    * on the exception
    */
  def validateNoThrow(id: Short, block: => Any, modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value): ValidationState[T] = {
  ...
  def validateTryFlatten(id: Short, operation: T => Try[T], condition: T => Boolean,
                         modifierId: ModifierId, modifierTypeId: ModifierTypeId.Value): ValidationState[T] = {
