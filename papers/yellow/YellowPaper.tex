...
\begin{itemize}
    ...
    candidate is serialized as a \coin{}~(see Section~\ref{box-format} for details), but without last field~(transaction
...
\begin{itemize}
    ...
    \item{} total transaction cost which consists of cost of all spending proofs verification (see "Cost Model" section
            in SigmaState specification) plus the cost of all tokens containing in transaction inputs and outputs
            ...
            should not be greater than a limit per block~(adjustable via miners voting).
