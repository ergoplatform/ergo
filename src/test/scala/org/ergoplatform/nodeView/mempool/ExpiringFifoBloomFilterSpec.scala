...
class ExpiringFifoBloomFilterSpec extends AnyFlatSpec with Matchers {
  ...
  it should "behave as fixed sized FIFO collection of bloom filters" in {
    val bf = ExpiringFifoBloomFilter.empty(500, 0.2)
    bf.fifoQueueSize shouldBe 5
    bf.approxElemCountPerBloomFilter shouldBe 100
    ...
    assert(fullBf.bloomFilterQueue.map(_._1) == Vector(4,3,2,1,0), "BF indexes must be 4-0")
    assert(fullBf.approximateElementCount > 430, "At least 430 elements must be present")

    // add some more elements over limit
    val newBf = (501 to 1000).map(_.toString).foldLeft(fullBf) { case (acc, n) => acc.put(n) }
    newBf.bloomFilterQueue.size shouldBe 5
    assert(newBf.bloomFilterQueue.map(_._1) == Vector(9,8,7,6,5), "BF indexes must be 9-5")
