...
In Bitcoin and all PoW-based currencies, a miner collects a number of unconfirmed transactions and builds a Merkle tree on top of them.
...
by trying to generate a block that pays the reward to the pool. In case of successful block generation, however, the miner generates and broadcasts another block that sends the reward to the miner instead of the pool.
...
At a technical level, both approaches rely on a miner's ability to steal the pool's rewards. The difference lies in the way this occurs. In Type 1 schemes, the miner is able to steal the reward {\em after} the block gets finalized.
