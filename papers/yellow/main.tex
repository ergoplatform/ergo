...
from the state during transaction application: monetary value, guard script, contents of all the registers, all the bytes and identifier of the \coin{}.

\subsection{\Coin{} Format}

A \coin{} is made of registers, we allow every \coin{} in the system to have up to 10 registers in the \ecash{} chain,
...
A transaction simply refers to a set of \coin{}s, by providing their identifiers and also authorization proofs for
...
This tree is to be constructed as follows. A data block under a leaf of the tree could be 1-byte or 64 bytes in length.
A block of 64 bytes contains identifier of the transaction(256-bits digest of transaction bytes without spending proofs)
and 256 bits of a digest of all the spending proofs combined. A leaf of 1 bytes means that it contains
$null$~(so does not contain any transaction).
