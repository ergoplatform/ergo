...
class ErgoMiner(ergoSettings: ErgoSettings,
                ...
                private var secretKeyOpt: Option[DLogProverInput]) extends Actor with ScorexLogging {
  ...
  override def preStart(): Unit = {
    if (secretKeyOpt.isEmpty) {
      ...
      viewHolderRef ! GetDataFromCurrentView[ErgoHistory, DigestState, ErgoWallet, ErgoMemPool, Unit] { v =>
        v.vault.firstSecret().onComplete(rTry => rTry.foreach(r => callback ! UpdateSecret(r)))(context.dispatcher)
      }
    }
  ...
  /** Whether current candidate block is outdated or absent
    */
  ...
  /** Are we ready to start mining after receiving the block given
    */
  private def shouldStartMining(b: ErgoFullBlock): Boolean = {
  ...
  private def receiveSemanticallySuccessfulModifier: Receive = {
    ...
    /**
      * Non obvious case when mining is enabled, but miner hasn't started yet. Initialization case.
      * We've received block that been generated by somebody else or genesis block while we hasn't started.
      * And this block was generated after our miner had been started. That means that we are ready
      * to start mining. This block could be either genesis or generated by another node.
      */
    case SemanticallySuccessfulModifier(mod: ErgoFullBlock) if shouldStartMining(mod) => self ! StartMining
