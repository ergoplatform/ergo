...
import org.ergoplatform.nodeView.history.extra.{
  BalanceInfo,
  IndexedErgoBox,
...
trait ApiExtraCodecs extends JsonCodecs {
  ...
  implicit val indexedTxSeqEncoder: Encoder[(Seq[IndexedErgoTransaction], Long)] = {
    iEtSeq =>
      Json.obj(
  ...
  implicit val balancesSnapshotEncoder: Encoder[WalletDigest] = { v =>
    ...
    Json.obj(
      ...
      "assets" -> walletAssetBalances.toMap
        .map(x => (x._1: String, x._2))
        .asJson //toMap to have assets as JSON map
    )
  }

  implicit val SnapshotInfoEncoder: Encoder[SnapshotsInfo] = { si =>
    Json.obj(
      "availableManifests" -> si.availableManifests.map {
        case (height, manifest) =>
          height -> manifest
      }.asJson
    )
  }
  ...
  implicit val SnapshotInfoDecoder: Decoder[SnapshotsInfo] = { cursor =>
    for {
      availableManifests <- Decoder
                             .decodeMap[Int, ManifestId]
                             .tryDecode(cursor.downField("availableManifests"))
