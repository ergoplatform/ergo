...
class ParametersSpecification extends ErgoPropertyTest {
  ...
  property("simple voting - start - conditions") {
    ...
    val esc2 = esc.processExtension(p, h, 2).get
    ...
    esc.processExtension(p, hw1, 2).isSuccess shouldBe false
    ...
    esc.processExtension(p, hw2, 2).isSuccess shouldBe false
    ...
    esc.processExtension(p, hw3, 2).isSuccess shouldBe false
    ...
    esc.processExtension(p, hw4, 2).isSuccess shouldBe false
    ...
    val esc30 = esc2.processExtension(p, he, 3).get
    val esc40 = esc30.processExtension(p, he, 4).get
    esc40.currentParameters.k shouldBe kInit

    //quorum gathered - parameter change
    val esc31 = esc2.processExtension(p, h, 3).get
    ...
    val esc41 = esc31.processExtension(p4, he, 4).get
