...
class InfoApiRoutesSpec extends AnyFlatSpec
  ...
  val statsCollector: ActorRef = ErgoStatsCollectorRef(nodeViewRef, networkControllerRef, null, settings, fakeTimeProvider, parameters)
  ...
  it should "return info" in {
    Get("/info") ~> route ~> check {
      ...
      c.downField("eip27Supported").as[Boolean] shouldEqual Right(false)
    }
  }
