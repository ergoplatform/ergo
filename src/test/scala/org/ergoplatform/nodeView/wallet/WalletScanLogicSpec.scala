...
class WalletScanLogicSpec extends ErgoCorePropertyTest with DBSpec with WalletTestOps {
  ...
  property("scanBlockTransactions") {
    withVersionedStore(10) { store =>
      ...
      forAll(trackedTransactionGen) { trackedTransaction =>
        ...
        val outputs2 = IndexedSeq(new ErgoBoxCandidate(spendingTx.outputs.map(_.value).sum, ErgoTree.fromProposition(FalseLeaf.toSigmaProp), height1))
  ...
  property("scan with forced flag is sharing boxes with the p2k-wallet") {
    val trueProp = ErgoTree.withSegregation(DefaultHeader, TrueLeaf.toSigmaProp)
