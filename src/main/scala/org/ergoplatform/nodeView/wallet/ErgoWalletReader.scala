...
trait ErgoWalletReader extends VaultReader {
  ...
  def transactions(offset: Int, limit: Int): Future[Seq[AugWalletTransaction]] =
    (walletActor ? GetTransactions(offset, limit)).mapTo[Seq[AugWalletTransaction]]
  ...
  def transactionsByScanId(scanId: ScanId, offset: Int, limit: Int): Future[ScanRelatedTxsResponse] =
    (walletActor ? GetScanTransactions(scanId, offset, limit)).mapTo[ScanRelatedTxsResponse]
