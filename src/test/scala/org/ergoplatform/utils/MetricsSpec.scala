...
import org.ergoplatform.nodeView.state.UtxoState.{appendFullBlockReporter, createUtxoStateReporter}
import org.ergoplatform.settings.ErgoAlgos
import org.ergoplatform.utils.metrics.{CsvCollector, Reporter, measureCostedOp, MeasuredObject, ApplyTransactionsReporter, measureOp}
import scalan.RType
...
class MetricsSpec extends ErgoPropertyTest {
  ...
  class TestCsvCollector extends CsvCollector(settings) {
  ...
  property("CsvCollector") {
    val collector = new TestCsvCollector
    val block = ErgoFullBlock(
      Header(
        1.toByte,
