...
class ErgoWalletSpec extends PropSpec with WalletTestOps {
  ...
  property("off-chain scan") {
    withFixture { implicit w =>
      ...
      val box1 = IndexedSeq(new ErgoBoxCandidate(balance1, pubKey, emptyStateContext.currentHeight, randomNewAsset))
      ...
      val box2 = IndexedSeq(new ErgoBoxCandidate(balance2, pubKey, emptyStateContext.currentHeight, randomNewAsset))
      ...
      val box3 = IndexedSeq(new ErgoBoxCandidate(balance3, Constants.TrueLeaf, emptyStateContext.currentHeight, randomNewAsset))
