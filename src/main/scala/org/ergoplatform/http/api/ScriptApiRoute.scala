...
case class ScriptApiRoute(readersHolder: ActorRef, ergoSettings: ErgoSettings)
  ...
  /**
   * Generic helper to create address routes (P2S or P2SH).
   * Extracts common boilerplate shared between p2sAddress and p2shAddress endpoints.
   *
   * @param pathName the path name for the route (e.g. "p2sAddress" or "p2shAddress")
   * @param addressCreator function creating the appropriate address type from ErgoTree
   */
  private def createAddressRoute(
    pathName: String,
    addressCreator: ErgoTree => ErgoAddress
  ): Route = {
    (path(pathName) & post & entity(as[CompileRequest])) { compileRequest =>
      withWalletAndStateOp(r =>
        (r.w.publicKeys(0, loadMaxKeys), r.s.stateContext.blockVersion)
      ) { case (addrsF, bv) =>
        onSuccess(addrsF) { addrs =>
          ...
          VersionContext.withVersions(scriptVersion, treeVersion) {
            compileSource(
              ...
              keysToEnv(addrs.map(_.pubkey)),
              treeVersion
            ).map(addressCreator).fold(
