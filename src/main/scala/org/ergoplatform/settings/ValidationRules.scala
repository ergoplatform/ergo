...
object ValidationRules {
  ...
  lazy val rulesSpec: Map[Short, (String => Invalid, Boolean, Seq[Class[_]])] = Map(
    ...
    alreadyApplied -> (s => fatal(s"A modifier should not be applied yet. $s"), true, Seq(classOf[Header], classOf[ADProofs], classOf[Extension], classOf[BlockTransactions])),


    // stateless transaction validation
    txNoInputs -> (s => fatal(s"A transaction should have at least one input. $s"), true, Seq(classOf[ErgoTransaction])),
    txNoOutputs -> (s => fatal(s"A transaction should have at least one output. $s"), true, Seq(classOf[ErgoTransaction])),
    txManyInputs -> (s => fatal(s"A transaction inputs number should not exceed ${Short.MaxValue}. $s"), true, Seq(classOf[ErgoTransaction])),
    txManyDataInputs -> (s => fatal(s"A transaction data inputs number should not exceed ${Short.MaxValue}. $s"), true, Seq(classOf[ErgoTransaction])),
    txManyOutputs -> (s => fatal(s"A transaction outputs number should not exceed ${Short.MaxValue}. $s"), true, Seq(classOf[ErgoTransaction])),
    ...
    txCost -> (s => fatal(s"The total cost of transaction scripts should not exceed <maxBlockCost>. $s"), true, Seq(classOf[ErgoTransaction])),
    ...
    txFuture -> (s => fatal(s"Every output of the transaction should have creationHeight less or equals than the height of the block with this transaction. $s"), true, Seq(classOf[ErgoTransaction])),
    ...
    txBoxToSpend -> (s => recoverable(s"Box id doesn't match the input. $s"), true, Seq(classOf[ErgoTransaction])),
    ...
    hdrGenesisFromConfig -> (s => fatal(s"Genesis header id should equal to id from the config. $s"), true, Seq(classOf[Header])),
    ...
    hdrVotes -> (s => fatal(s"A header should contain three votes, with no duplicates and contradictory votes, that should be known by parametersDescs. $s"), true, Seq(classOf[Header])),
    hdrNonIncreasingTimestamp -> (s => fatal(s"Header timestamp should be greater than parents. $s"), true, Seq(classOf[Header])),
    hdrHeight -> (s => fatal(s"A header height should be greater by one than parents. $s"), true, Seq(classOf[Header])),
    hdrPoW -> (s => fatal(s"A header should contain the correct PoW solution. $s"), true, Seq(classOf[Header])),
    ...
    bsCorrespondsToHeader -> (s => fatal(s"Block sections should correspond to the declared header. $s"), true, Seq(classOf[ADProofs], classOf[Extension], classOf[BlockTransactions])),
    bsHeaderValid -> (s => fatal(s"A header for the block section should not be marked as invalid. $s"), true, Seq(classOf[ADProofs], classOf[Extension], classOf[BlockTransactions])),
    ...
    bsTooOld -> (s => fatal(s"Block section should correspond to a block header that is not pruned yet. $s"), true, Seq(classOf[ADProofs], classOf[Extension], classOf[BlockTransactions])),
    fbOperationFailed -> (s => fatal(s"Operations applied to AVL+ tree should pass. $s"), true, Seq(classOf[ErgoFullBlock])),
    fbDigestIncorrect -> (s => fatal(s"Calculated AVL+ digest should be equal to the declared one. $s"), true, Seq(classOf[ErgoFullBlock])),
    ...
    exIlStructure -> (s => fatal(s"Interlinks should have the correct structure. $s"), true, Seq(classOf[Extension])),

    exKeyLength -> (s => fatal(s"Extension fields key length should be ${Extension.FieldKeySize}. $s"), true, Seq(classOf[Extension])),
    exValueLength -> (s => fatal(s"Extension field value length should be <= ${Extension.FieldValueMaxSize}. $s"), true, Seq(classOf[Extension])),
    exDuplicateKeys -> (s => fatal(s"An extension should not contain duplicate keys. $s"), true, Seq(classOf[Extension])),
