...
class ErgoMiner(
  ...
  def starting(minerState: MinerState): Receive = {
    ...
    case FullBlockApplied(header) if shouldStartMine(header) =>
      viewHolderRef ! GetDataFromCurrentView[ErgoHistory, Unit] { v =>
        if (v.history.isHeadersChainSynced) {
          log.info("Starting mining triggered by incoming block")
          self ! StartMining
        } else {
          log.info("Mining is enabled but node is not yet synced. Mining will start automatically once synchronization completes.")
        }
      }
