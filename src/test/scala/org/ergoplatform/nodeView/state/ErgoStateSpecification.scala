...
class ErgoStateSpecification extends ErgoPropertyTest {
  ...
  property("applyModifier() - double spending") {
    forAll(boxesHolderGen, Gen.choose(1: Byte, 2: Byte)) { case (bh, version) =>
      ...
      us.applyModifier(doubleSpendBlock, None) shouldBe 'failure
      us.applyModifier(validBlock, None) shouldBe 'success

      ds.applyModifier(doubleSpendBlock, None) shouldBe 'failure
      ds.applyModifier(validBlock, None) shouldBe 'success
    }
  }
