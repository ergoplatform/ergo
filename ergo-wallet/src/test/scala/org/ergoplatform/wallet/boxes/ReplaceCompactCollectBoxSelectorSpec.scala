...
class ReplaceCompactCollectBoxSelectorSpec extends PropSpec with Matchers with EitherValues {
  ...
  def trackedBox(value:Long) = TrackedBox(parentTx, 0, None, box(value), Set(PaymentsAppId))
