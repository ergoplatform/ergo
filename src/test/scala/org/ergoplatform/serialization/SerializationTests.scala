...
import org.ergoplatform.nodeView.wallet.persistence.RegistrySummarySerializer
...
class SerializationTests extends ErgoPropertyTest with WalletGenerators with scorex.testkit.SerializationTests {
  ...
  property("RegistryIndex serialization") {
    forAll(registrySummaryGen) { index =>
      RegistrySummarySerializer.parseBytes(RegistrySummarySerializer.toBytes(index)) shouldEqual index
    }
  }
