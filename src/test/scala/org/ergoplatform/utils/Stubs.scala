...
trait Stubs extends ErgoGenerators with ErgoTestHelpers with ChainGenerator with TestFileUtils {
  ...
  class WalletActorStub extends Actor {
    ...
    def receive: Receive = {
      ...
      case AddScan(req) =>
        val scanId = ScanId @@ (apps.lastOption.map(_._1).getOrElse(100) + 1)
  ...
  object WalletActorStub {
    ...
    val secretKey = ExtendedSecretKey.deriveMasterKey(Mnemonic.toSeed(SecretString.create(mnemonic)), usePre1627KeyDerivation = true).derive(path)
