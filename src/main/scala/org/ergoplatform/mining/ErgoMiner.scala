...
class ErgoMiner(
  ...
  /** Check if blockchain is almost synced (headers height is within 6 blocks of full blocks height) */
  private def isBlockchainNearlySynced(headersHeight: Int, fullBlockHeight: Int): Boolean = {
    headersHeight < fullBlockHeight + 6
  }
  ...
  def starting(minerState: MinerState): Receive = {
    ...
    case FullBlockApplied(header) if shouldStartMine(header) =>
      log.info(s"Block ${header.id} applied, checking sync status for mining")
      self ! StartMining
