...
The parties can than locally compute the Shamir sharing $[d]$ of $d = rx+s$ using the relation $d_i = rx_i+s_i$ for party $i$.
The parties then post the shares $d_i$ on the public bulletin board so that everyone can compute $d$ in the clear. This is the step that requires a large public broadcast. 
