...
A transaction is a transfer of Erg value and tokens that is broadcast to the network and collected into blocks.
A transaction references \coin{} from the state and dedicates input Erg and token values to new \coin{}.
...
be $(creation\_height, transaction\_id || output\_index)$ for a box  and $(creation\_height, 0^{34*8})$~(so instead
of real transaction id and output index, a zero-bit string of 34 bytes is used).
...
\begin{itemize}
    ...
    \item{\em outputs} - new \coins{} that will be included into the state during the transaction application.
\end{itemize}
...
Transaction id is calculated as a Blake2b256 hash from messages to sign.
