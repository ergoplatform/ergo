...
class CrawlerRunner(args: Array[String]) extends Application {
  ...
  lazy val ergoSettings: ErgoSettings = ErgoSettings.read(cfgPath).copy(directory = tempDir.getAbsolutePath)
  ...
  crawlerRef ! SubTo(nodeViewHolderRef, Seq(EventType.SuccessfulSyntacticallyValidModifier))
