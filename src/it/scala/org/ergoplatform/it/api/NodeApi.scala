...
trait NodeApi {
  ...
  def connectedPeers: Future[Seq[Peer]] = get("/peers/connected").map { r =>
    Json.fromString(r.getResponseBody).hcursor.downField("peers").as[Seq[Peer]].right.get
  }

  def blacklistedPeers: Future[Seq[BlacklistedPeer]] = get("/peers/blacklisted").map { r =>
    Json.fromString(r.getResponseBody).as[Seq[BlacklistedPeer]].right.get
  }
  ...
  def height: Future[Int] = get("/blocks/height").as[Json].map(v => v.hcursor.downField("height").as[Int].right.get)
