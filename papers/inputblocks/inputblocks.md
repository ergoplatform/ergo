...
of transactions. We set *t = T × subBlocksPerBlock*, where *subBlocksPerBlock* is a votable network parameter
(id 9, range 2–2048), and define input-block *ib* generation condition as *H(ib) < t*.
Target is inversely proportional to difficulty, so multiplying the target by *subBlocksPerBlock* is equivalent
to dividing the difficulty by *subBlocksPerBlock*.
For example, with *subBlocksPerBlock = 64*, a miner can generate on average 63 input blocks plus an ordering block
