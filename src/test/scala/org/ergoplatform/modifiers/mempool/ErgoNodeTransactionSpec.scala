...
class ErgoNodeTransactionSpec extends ErgoCorePropertyTest {
  ...
  private def checkTx(from: IndexedSeq[ErgoBox], tx: ErgoTransaction): Try[Int] = {
    tx.statelessValidity()
      .flatMap(_ => tx.statefulValidity(from, emptyDataBoxes, emptyStateContext))
  }

  private def modifyValue(
  ...
  private def modifyAsset(
    ...
    val tokens = boxCandidate.additionalTokens.map {
      case (id, amount) =>
  ...
  private def updateAnAsset(
    tx: ErgoTransaction,
    from: IndexedSeq[ErgoBox],
    deltaFn: Long => Long
  ) = {
    val updCandidates = tx.outputCandidates
      .foldLeft(IndexedSeq[ErgoBoxCandidate]() -> false) {
        case ((seq, modified), ebc) =>
          if (modified) {
            (seq :+ ebc) -> true
          } else {
            if (ebc.additionalTokens.nonEmpty && ebc.additionalTokens.exists(t =>
                  !java.util.Arrays.equals(t._1.toArray, from.head.id)
                )) {
  ...
  property("monotonic creation height") {
    def stateContext(height: Int, blockVersion: Byte): ErgoStateContext = {
      val header =
        defaultHeaderGen.sample.get.copy(version = blockVersion, height = height)
      val params = Parameters(
        LaunchParameters.height,
    ...
    val txGen = boxesGenTemplate(
      minAssets      = 0,
      maxAssets      = 5,
      minInputs      = 5,
      maxInputs      = 10,
      propositionGen = trueLeafGen
    ).map {
        case (boxes, _) =>
      ...
      .map { boxes =>
        ...
        val unsignedTx = if (fixed) {
          preUnsignedTx.copy(outputCandidates = preUnsignedTx.outputCandidates.map {
            out =>
        ...
        val tx = defaultProver
          .sign(unsignedTx, boxes, IndexedSeq.empty, emptyStateContext)
    ...
    forAll(txGen) {
      case (boxes, tx, fixed) =>
        ...
        tx.statefulValidity(
            boxes,
            IndexedSeq.empty,
            stateContextForTx(tx, blockVersion = 1)
          )
        ...
        tx.statefulValidity(
            boxes,
            IndexedSeq.empty,
            stateContextForTx(tx, blockVersion = 2)
          )
          .isSuccess shouldBe true
        ...
        if (fixed) {
          ...
          val expectedMessage = ValidationRules.errorMessage(
            ValidationRules.txMonotonicHeight,
            "",
            emptyModifierId,
  ...
  property("a valid transaction is valid") {
    forAll(validErgoTransactionGen) {
      case (from, tx) =>
        ...
        tx.statefulValidity(from, emptyDataBoxes, emptyStateContext)
          .isSuccess shouldBe true
    }
  }

  property("ergo preservation law holds") {
    forAll(validErgoTransactionGen, smallPositiveInt) {
      case ((from, tx), deltaAbs) =>
        ...
        wrongTx
          .statefulValidity(from, emptyDataBoxes, emptyStateContext)
  ...
  property("impossible to create a negative-value output") {
    forAll(validErgoTransactionGen) {
      case (from, tx) =>
        val negValue = Math.min(
          Math.abs(Random.nextLong()),
          Long.MaxValue - tx.outputCandidates.head.value
        )
        val wrongTx = tx.copy(outputCandidates =
          modifyValue(
            tx.outputCandidates.head,
            -(tx.outputCandidates.head.value + negValue)
  ...
  property("assets preservation law holds") {
    forAll(validErgoTransactionWithAssetsGen) {
      case (from, tx) =>
  ...
  property("impossible to overflow an asset value") {
    ...
    forAll(gen) {
      case (from, tx) =>
        val tokenOpt = tx.outputCandidates
        ...
        whenever(tokenOpt.nonEmpty) {
          ...
          val updCandidates = tx.outputCandidates.map { c =>
            val updTokens = c.additionalTokens.map {
              case (id, amount) =>
      ...
      case (from, tx) =>
        ...
        e.getMessage should startWith(
          ValidationRules.errorMessage(
            ValidationRules.txScriptValidation,
            "",
            emptyModifierId,
            ErgoTransaction.modifierTypeId
          )
  ...
  property("assets usage correctly affects transaction total cost") {
    ...
    forAll(txGen) {
      case (from, tx) =>
        ...
        val randomToken =
          (scorex.util.Random.randomBytes().toTokenId, Random.nextInt(100000000).toLong)

        val in0 = from.last
        // new token added to the last input
        val modifiedIn0 = testBox(
          in0.value,
          in0.ergoTree,
          in0.creationHeight,
          in0.additionalTokens.toArray.toSeq :+ randomToken,
          in0.additionalRegisters,
          in0.transactionId,
          in0.index
        )
        ...
        val modifiedIn1 = testBox(
          in1.value,
          in1.ergoTree,
          in1.creationHeight,
          in1.additionalTokens.toArray.toSeq :+ existingToken,
          in1.additionalRegisters,
          in1.transactionId,
          in1.index
        )
        ...
        val modifiedOut0 = testBox(
          out0.value,
          out0.ergoTree,
          out0.creationHeight,
          out0.additionalTokens.toArray.toSeq :+ randomToken,
          out0.additionalRegisters,
          out0.transactionId,
          out0.index
        )
        // existing token added to the last output
        val modifiedOut1 = testBox(
          out0.value,
          out0.ergoTree,
          out0.creationHeight,
          out0.additionalTokens.toArray.toSeq :+ existingToken,
          out0.additionalRegisters,
          out0.transactionId,
          out0.index
        )
        ...
        val txMod2 = tx.copy(
          inputs           = tx.inputs.init :+ txInMod0, // new token group added to one input and one output
          outputCandidates = tx.outputCandidates.init :+ modifiedOut0
        )
        val txMod3 = tx.copy(
          inputs           = tx.inputs.init :+ txInMod1, // existing token added to one input and one output
          outputCandidates = tx.outputCandidates.init :+ modifiedOut1
        )

        val inputIncTxCost0 = txMod0
          .statefulValidity(from.init :+ modifiedIn0, emptyDataBoxes, emptyStateContext)
          .get
        val inputIncTxCost1 = txMod1
          .statefulValidity(from.init :+ modifiedIn1, emptyDataBoxes, emptyStateContext)
          .get
        val outputIncTxCost0 = txMod2
          .statefulValidity(from.init :+ modifiedIn0, emptyDataBoxes, emptyStateContext)
          .get
        val outputIncTxCost1 = txMod3
          .statefulValidity(from.init :+ modifiedIn1, emptyDataBoxes, emptyStateContext)
          .get
  ...
  property(
    "spam simulation (transaction validation cost with too many tokens exceeds block limit)"
  ) {
    ...
    val tokens =
      (0 until 255).map(_ => (scorex.util.Random.randomBytes().toTokenId, Random.nextLong)
      )
    val (in, out) = {
      val in0  = inputs.head
    ...
    validFailure.failed.get.getMessage should startWith(
      ValidationRules
        .errorMessage(
          txAssetsInOneBox,
          "",
  ...
  property("transaction with too many inputs should be rejected") {
    ...
    val (validity, time0) = BenchmarkUtil.measureTime(
      tx.statefulValidity(from, IndexedSeq(), emptyStateContext)
    )
    validity.isSuccess shouldBe false
    assert(time0 <= Timeout)

    val cause = validity.failed.get.getMessage
    cause should startWith(
      ValidationRules
        .errorMessage(
          bsBlockTransactionsCost,
          "",
          emptyModifierId,
          ErgoTransaction.modifierTypeId
        )
    ...
    val ps = Parameters(
      0,
      DefaultParameters.updated(MaxBlockCostIncrease, maxCost),
      emptyVSUpdate
    )
    val sc = new ErgoStateContext(
      Seq.empty,
      None,
      genesisStateDigest,
      ps,
      ...
      .upcoming(
        org.ergoplatform.mining.group.generator,
        0L,
  ...
  property("transaction cost") {
    ...
    println(
      s"initialCost + totalAssetsAccessCost: ${initialCost + totalAssetsAccessCost}"
    )
    val approxCost: Int =
      (initialCost + totalAssetsAccessCost + scriptsValidationCosts).toInt
    ...
    val calculatedCost = tx
      .statefulValidity(from, IndexedSeq(), sc)(ErgoInterpreter(sc.currentParameters))
      .get
    ...
    tx.statefulValidity(from, IndexedSeq(), sc, accumulatedCost = 1)(
      ErgoInterpreter(sc.currentParameters)
    ) shouldBe 'failure
  }
  ...
  property("cost accumulated correctly across inputs") {
    ...
    def inputCost(tx: ErgoTransaction, from: IndexedSeq[ErgoBox]): Long = {
      ...
      val inputCost =
        verifier.verify(from(idx).ergoTree, ctx, proof, messageToSign).get._2

      inputCost
    }

    forAll(smallPositiveInt) { inputsNum =>
      val nonTrivialTrueGen =
        Gen.const(AND(Seq(TrueLeaf, TrueLeaf)).toSigmaProp.treeWithSegregation)
      ...
      val txCost =
        tx.statefulValidity(from, IndexedSeq(), emptyStateContext, accInitCost).get

      val (inAssets, inAssetsNum): (Map[Seq[Byte], Long], Int) =
        ErgoBoxAssetExtractor.extractAssets(from).get
      val (outAssets, outAssetsNum): (Map[Seq[Byte], Long], Int) =
        ErgoBoxAssetExtractor.extractAssets(tx.outputs).get
      ...
      val unsignedTx =
        UnsignedErgoTransaction(tx.inputs, tx.dataInputs, tx.outputCandidates)
      val signerTxCost =
        defaultProver
          .signInputs(
            unsignedTx,
