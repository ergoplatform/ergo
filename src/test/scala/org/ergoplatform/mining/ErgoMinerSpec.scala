...
class ErgoMinerSpec extends AnyFlatSpec with ErgoTestHelpers with Eventually {
  ...
  override protected def withFixture(test: NoArgTest): org.scalatest.Outcome = {
    info(s"Mining fixture network=${defaultSettings.networkType.verboseName}, launchVersion=${defaultSettings.launchParameters.blockVersion}")
  ...
  it should "not include too costly transactions" in new TestKit(newActorSystem()) {
    ...
    rejectionProbe.expectMsgPF(newBlockDelay) {
      case FailedTransaction(rejected, error) =>
        rejected.id shouldBe costlyTx.id
        error shouldBe a[TooHighCostError]
        error.asInstanceOf[TooHighCostError].txCost shouldBe Some(439080)
    }
  ...
  it should "include only one transaction from 2 spending the same box through internal solution submission" in new TestKit(newActorSystem()) {
    val testProbe = new TestProbe(system)
    system.eventStream.subscribe(testProbe.ref, newBlockSignal)
    val ergoSettings: ErgoSettings = defaultSettings.copy(directory = createTempDir.getAbsolutePath)
    ...
    testProbe.expectMsgPF(candidateGenDelay) {
      case StatusReply.Success(candidate: Candidate) =>
        // The generator replies directly to the forwarded request. Native mining
        // threads submit solutions to this same actor.
        val candidateGenerator = testProbe.lastSender
        ...
        // The reply and application event may arrive in either order. Other
        // native mining threads may also apply blocks while this solution is handled.
        ...
        testProbe.fishForMessage(blockValidationDelay) {
          ...
          case FullBlockApplied(header) =>
            applied = applied || header.id == block.header.id
