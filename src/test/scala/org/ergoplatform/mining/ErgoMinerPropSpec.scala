...
class ErgoMinerPropSpec extends ErgoPropertyTest {
  ...
  property("should only collect valid transactions") {
    def checkCollectTxs(maxCost: Long, maxSize: Int, withTokens: Boolean = false): Unit = {
      ...
      val upcomingContext = us.stateContext.upcoming(h.minerPk, h.timestamp, h.nBits, h.votes, Seq(), h.version, None)
      upcomingContext.currentHeight shouldBe (us.stateContext.currentHeight + 1)

      val fromSmallMempool = ErgoMiner.collectTxs(defaultMinerPk, maxCost, maxSize, us, upcomingContext, Seq(head), Seq())(validationSettings)._1
      fromSmallMempool.size shouldBe 2
      fromSmallMempool.contains(head) shouldBe true

      val fromBigMempool = ErgoMiner.collectTxs(defaultMinerPk, maxCost, maxSize, us, upcomingContext, txsWithFees, Seq())(validationSettings)._1
