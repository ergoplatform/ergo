...
class MatrixFeeSelectionSpec extends ErgoCorePropertyTest with ErgoCompilerHelpers {
  private val states = scala.collection.mutable.ArrayBuffer.empty[UtxoState]

  override protected def withFixture(test: NoArgTest): Outcome = {
    try super.withFixture(test)
    finally {
      try states.foreach(_.closeStorage())
      finally states.clear()
    }
  }
  ...
  private def state(boxes: Seq[ErgoBox], protocolCost: Int = parameters.maxBlockCost): UtxoState = {
    val created = UtxoState.fromBoxHolder(BoxHolder(boxes), None, createTempDir(), settings,
      parameters.withBlockCost(protocolCost))
    states += created
    created
  }
