...
\subsubsection{More Examples}
 ...
 We briefly give a light on few more examples along with links document providing details and code/

\paragraph{Atomic Swap}

Cross-chain atomic swap could be done in the same way as proposed for Bitcoin, see initial protocol for Bitcoin in
...
\paragraph{Crowdfunding}
 ...
 We consider a simplest crowdfunding scenario. In the example, a crowdfunding project is considered successful if it
 can collect unspent outputs with total value not less than a certain amount before
 a certain height. A project backer creates an output box protected by the following statement: \knote{finish}
