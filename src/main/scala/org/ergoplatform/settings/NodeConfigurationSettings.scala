...
trait NodeConfigurationReaders extends StateTypeReaders with ModifierIdReader {
  ...
  implicit val nodeConfigurationReader: ValueReader[NodeConfigurationSettings] = { (cfg, path) =>
    ...
    NodeConfigurationSettings(
      ...
      cfg.as[Int](s"$path.invalidModifiersBloomFilterCapacity"),
      cfg.as[Double](s"$path.invalidModifiersBloomFilterExpirationRate"),
      cfg.as[Int](s"$path.invalidModifiersCacheSize"),
      cfg.as[FiniteDuration](s"$path.invalidModifiersCacheExpiration"),
