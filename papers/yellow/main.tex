...
A leaf is $hash((pos +1) || data)$, if the $data$ is not empty
~(we do add $pos + 1$ prefix for domain separation), or $null$ otherwise. For internal nodes, a node is
$hash(0 || left\_child || right\_child)$, if either left child or right child of the node is not $null$, $null$ othewise.
