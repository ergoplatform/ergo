...
class StateRecoveryDigestNodeSpec extends FreeSpec with IntegrationSuite {
  ...
  "Startup with only history available" in {
    ...
    val minerNode: Node = docker.startTestNetNode(minerConfig, specialVolumeOpt = Some((minerLocalVolume, remoteVolume))).get
    ...
    val result = Async.async {
      ...
      val nodeForSyncing: Node = docker
        .startTestNetNode(minerConfig, specialVolumeOpt = Some((minerLocalVolume, remoteVolume))).get
      Async.await(nodeForSyncing.waitForHeight(approxMinerTargetHeight + 2))

      val followerNode: Node = docker
        .startTestNetNode(followerConfig, specialVolumeOpt = Some((followerLocalVolume, remoteVolume))).get
