...
object DifficultyControlChecker extends App with ErgoGenerators {
  ...
  case class DiffRow(height: Int, requiredDifficulty: BigInt, timeDiff: Long) {
  ...
  val data: Seq[DiffRow] = headers.sliding(2).map { p =>
    ...
    DiffRow(end.height, end.requiredDifficulty, end.timestamp - start.timestamp)
  }.toSeq

  println(s"height,requiredDifficulty,realDifficulty")
