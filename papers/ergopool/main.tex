...
\title{Bypassing Non-Outsourceabile Proof-of-Work Schemes Using Collateralized Smart Contracts}
...
% In particular 
the reward is stolen {\em before} a block is finalized into the block-chain. 
...
One real-world implementation of this concept is \powname \cite{autolykos}, the PoW algorithm of Ergo. \powname uses a variation of Schnorr signatures, where the goal of a miner is to output $d$ such that $d < 2^{256}/\lambda$ and $\lambda$ is the difficulty parameter. The value $d$ is to be computed as follows. First compute $r = H(m\|n\|p\|w)$ where $m$ is the transactions digest, $n$ is a nonce, $p$ is a public key (an elliptic curve group element) and $w$ is an ephemeral public key that should never be reused in two different blocks. Let $x$ be the corresponding private key of $w$. Compute $d = xr - s$, where $s$ is the private key corresponding to $p$.
